...
Vue.filter('formatMarginNumber', function(amount, numberOfDecimalDigits) {
  return Utils.formatMarginNumber(amount, numberOfDecimalDigits);
});
...
Vue.filter('transactionStatus', function (status) {
  if (status== 'cancelled') return window.i18n.t('common.order_status.canceled')
  else
